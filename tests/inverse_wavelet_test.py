...
def test_inverse_wavelets() -> None:
    ...
    if EXACT_MATCH:
        ...
        assert np.allclose(signal_freq, signal_freq_in, atol=1.0e-12, rtol=1.0e-12)
        print('input wavelet domain data matches input frequency domain data')

        assert np.allclose(signal_time, signal_time_in, atol=1.0e-14, rtol=1.0e-15)
        print('input wavelet domain data matches input time domain data')

    # some additional internal consistency checkes
    assert np.allclose(signal_time_trans, signal_time, atol=1.0e-4, rtol=1.0e-10)
    print('expected level of internal consistency between time domain representations')

    assert np.allclose(np.real(signal_freq_trans), np.real(signal_freq), atol=1.0e-2, rtol=1.0e-2)
    assert np.allclose(np.imag(signal_freq_trans), np.imag(signal_freq), atol=1.0e-2, rtol=1.0e-2)
    assert np.allclose(signal_freq_trans, signal_freq, atol=1.0e-2, rtol=1.0e-2)
