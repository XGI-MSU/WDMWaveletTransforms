...
def test_harness_writes() -> None:
    ...
    os.system(
        'forward_wavelet_freq_harness '
        + str(filename_freq_in)
        + ' '
        + str(filename_fw_out)
        + ' '
        + str(dt)
    ...
    os.system(
        'forward_wavelet_time_harness '
        + str(filename_time_in)
        + ' '
        + str(filename_tw_out)
        + ' '
        + str(dt)
    ...
    os.system(
        'forward_wavelet_freq_time_harness '
        + str(filename_time_in)
        + ' '
        + str(filename_tfw_out)
        + ' '
        + str(dt)
        + ' '
        + str(Nt)
    ...
    os.system(
        'inverse_wavelet_time_harness '
        + str(filename_wavelet_in)
        + ' '
        + str(filename_wt_out)
        + ' '
        + str(dt)
    ...
    os.system(
        'inverse_wavelet_freq_time_harness ' + str(filename_wavelet_in) + ' ' + str(filename_wft_out) + ' ' + str(dt),
    )
    ...
    assert np.allclose(tfw_out, fw_in, atol=1.0e-4, rtol=1.0e-6)
    assert np.allclose(tfw_out, tw_in, atol=1.0e-4, rtol=1.0e-6)
    assert np.allclose(tw_out, fw_in, atol=1.0e-6, rtol=1.0e-6)

    assert np.allclose(tfw_out, w_in, atol=1.0e-4, rtol=1.0e-6)
    assert np.allclose(fw_out, w_in, atol=1.0e-14, rtol=1.0e-14)
    assert np.allclose(tw_out, w_in, atol=1.0e-6, rtol=1.0e-6)

    assert np.allclose(t_out1, t_out2, atol=1.0e-4, rtol=1.0e-6)
    assert np.allclose(t_in, t_out2, atol=1.0e-4, rtol=1.0e-6)
    if EXACT_MATCH:
        ...
        assert np.allclose(f_inr, f_outr, atol=1.0e-12, rtol=1.0e-12)
        assert np.allclose(f_ini, f_outi, atol=1.0e-12, rtol=1.0e-12)
        assert np.allclose(f_in, f_out, atol=1.0e-12, rtol=1.0e-12)
        assert np.allclose(t_out1, t_in, atol=1.0e-13, rtol=1.0e-13)
        assert np.allclose(fw_in, fw_out, atol=1.0e-13, rtol=1.0e-13)
        assert np.allclose(tw_in, tw_out, atol=1.0e-13, rtol=1.0e-13)
