...
def test_inverse_wavelets() -> None:
    ...
    assert np.allclose(wave_freq_in, wave_in, atol=1.0e-14, rtol=1.0e-15)

    # time domain forward and inverse transforms are not quite as accurate so these have a little more tolerance
    assert np.allclose(wave_in, wave_time_in, atol=1.0e-6, rtol=1.0e-6)
    assert np.allclose(wave_freq_in, wave_time_in, atol=1.0e-6, rtol=1.0e-6)
    ...
    if EXACT_MATCH:
        ...
        assert np.allclose(wave_freq_in, wave_freq_got, atol=1.0e-14, rtol=1.0e-15)
        print('forward frequency domain transform matches expectation closely')

        assert np.allclose(wave_time_in, wave_time_got, atol=1.0e-14, rtol=1.0e-15)
        print('forward time domain transform matches expectation closely')

    # still expect good match within the given architecture
    assert np.allclose(wave_freq_got, wave_time_got, atol=1.0e-6, rtol=1.0e-6)
