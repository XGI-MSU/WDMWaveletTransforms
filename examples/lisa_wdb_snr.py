...
def main():
    ...
    h_wavelet = transform_wavelet_time(h_time.data, Nf=Nf, Nt=Nt) * dt * np.sqrt(2)
    time_grid, freq_grid = get_wavelet_bins(duration, ND, Nf, Nt)
    hf = h_signal_f.data

    # h_wavelet = from_time_to_wavelet(h_time, Nt=Nt)
    psd_wavelet = evolutionary_psd_from_stationary_psd(psd, freq, freq_grid, time_grid) * dt
    snr2 = (4*dt/ND) * np.sum(np.conj(hf)*hf/psd)
    wavelet_snr2 = compute_wavelet_snr(h_wavelet, psd_wavelet)**2

    print(f"SNR2 in time domain (ND:{ND}): {snr2:.2f}")
    print(f"SNR2 in wavelet domain (Nf{Nf}xNt{Nt}): {wavelet_snr2:.2f}")
    ...
    axes[1].pcolor(
        time_grid, freq_grid, h_wavelet.T, cmap="RdBu", norm=TwoSlopeNorm(vcenter=0)
    )
    axes[1].set_ylim(1e-4, 1e-2)
    axes[1].set_xlabel("Time [s]")
    axes[1].set_ylabel("Frequency [Hz]")
    axes[0].text(
        0.1, 0.85, f"SNR: {snr:.2f}", transform=axes[0].transAxes, fontsize="x-large"
    )
    axes[1].text(
        0.1,
        0.85,
        f"SNR: {wavelet_snr2:.2f}",
